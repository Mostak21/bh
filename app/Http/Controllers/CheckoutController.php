        ...
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('combined_order_id') != null) {
                if ($request->payment_option == 'paypal') {
                    ...
                    if (addon_is_activated('otp_system') && !Auth::user()->email && !Auth::user()->phone) {
                        flash(translate('Your account should be verified before order'))->warning();
...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            $partial_payment = ceil($total/2);
            $cartItem['partial_payment']= $partial_payment;
			
			if($shipping_info['state_id']==348)
			{ $cod_status=1 ;
			}
            return view('frontend.payment_select', compact('carts', 'shipping_info', 'partial_payment','cod_status', 'total'));
